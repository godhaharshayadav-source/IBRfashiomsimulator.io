    ...
    <div class="container">
        ...
        <div id="scene5" class="scene flex flex-col items-center justify-center p-12">
            ...
            <div class="mt-12 flex flex-col items-center gap-4">
                <button id="generate-summary-btn" class="hidden p-3 font-orbitron bg-cyan-400 text-black w-72 hover:bg-cyan-300 transition-all duration-300">✨ GENERATE EXECUTIVE SUMMARY</button>
                <button id="impact-restart-btn" class="p-3 font-orbitron bg-transparent border-2 border-cyan-400 text-cyan-400 w-72 hover:bg-cyan-400 hover:text-black transition-all duration-300">RUN NEW SIMULATION</button>
            </div>
        </div>
        ...
        <div id="summary-modal" class="hidden absolute inset-0 z-10 p-12 flex items-center justify-center">
            <div class="absolute inset-0 bg-black/70 backdrop-blur-sm cursor-pointer" id="modal-overlay"></div>
            <div class="relative vision-card w-full max-w-2xl max-h-[80vh] flex flex-col p-8">
                <button id="modal-close-btn" class="absolute top-4 right-4 text-3xl text-cyan-400 hover:text-white">&times;</button>
                <h2 class="font-orbitron text-2xl glow">Executive Summary</h2>
                <div id="summary-content" class="mt-6 overflow-y-auto text-lg leading-relaxed">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentScene = 1;
    const simulationState = { trendName: '' };
    ...
    function switchScene(sceneNumber) {
        ...
        if (nextScene) {
            ...
            if (sceneNumber === 5) {
                ...
                }, 2000); // Reveal button after impact bars animate
            }
        }
    }
    ...
    async function runVisionScene() {
        ...
        try {
            ...
            simulationState.trendName = trendName.replace(/\*/g, '');

            const productName = "a futuristic jacket";
            const imagePrompt = `Editorial fashion photo of a ${productName} inspired by the "${simulationState.trendName}" trend. Dark, edgy, holographic, neon details, on a high-fashion model, cinematic lighting.`;
            
            const [imageUrl] = await Promise.all([ callGeminiImage(imagePrompt) ]);

            document.getElementById('vision-title').textContent = `${simulationState.trendName}: The ${productName}`;
    ...
    function runForecastScene() {
        switchScene(3);
        setTimeout(() => { document.getElementById('forecast-summary').style.opacity = '1'; }, 2000);
        setTimeout(() => { document.getElementById('forecast-next-btn').style.opacity = '1'; }, 3000);
    }

    // --- SCENE 4: PATH ---
    const pathCanvas = document.getElementById('path-canvas');
    const pathCtx = pathCanvas.getContext('2d');
    let supplyNodes = [], connections = [], cargo = {x: 0, y: 0, targetIndex: 1, speed: 1}, disruption = false;
    ...
    function setupPathScene() {
        ...
        cargo.x = supplyNodes[0].x; cargo.y = supplyNodes[0].y; cargo.targetIndex = 1;
        disruption = false;
    ...
    function animatePath() {
        ...
        let dx = targetNode.x - cargo.x, dy = targetNode.y - cargo.y;
        let dist = Math.sqrt(dx * dx + dy * dy);

        if (dist < cargo.speed) {
            if(cargo.targetIndex === 1 && disruption) { cargo.targetIndex = 2; } 
            else if (cargo.targetIndex < 4) { cargo.targetIndex = cargo.targetIndex === 1 || cargo.targetIndex === 2 ? 3 : cargo.targetIndex + 1; } 
            else { switchScene(5); return; }
        } else {
        ...
        pathCtx.shadowColor = '#FFD700'; pathCtx.shadowBlur = 15;
        pathCtx.fill();
    ...
    async function handleGenerateSummary() {
        summaryContent.innerHTML = `<div class="loader mx-auto"></div>`;
        summaryModal.classList.remove('hidden');
        try {
            const summaryPrompt = `The simulation for the '${simulationState.trendName}' fashion trend was a success. Key outcomes were: +32% profit growth, 18.2 tonnes of textile waste avoided, and 41.5 tonnes of CO2 emissions saved. Write a brief executive summary (2-3 sentences) for a business report highlighting these achievements and the role of AI-driven forecasting.`;
            const summaryText = await callGeminiText(summaryPrompt);
    ...
    document.getElementById('forecast-next-btn').addEventListener('click', () => { switchScene(4); setupPathScene(); });
